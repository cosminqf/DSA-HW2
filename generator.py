...
def generate_insert_heavy(filename, ops=1000, heaps=5, max_val=1000000):
    operations = []
    for _ in range(ops):
...
def generate_merge_heavy(filename, ops=1000, heaps=10, max_val=1000000):
    operations = []
    for _ in range(ops):
...
def generate_extract_heavy(filename, ops=1000, heaps=5, max_val=1000000):
    operations = []
    for _ in range(ops):
        h = random.randint(1, heaps)
        val = random.randint(1, max_val)
...
if __name__ == "__main__":
    generate_insert_heavy("insert_heavy.in", ops=999)
    generate_merge_heavy("merge_heavy.in", ops=999)
    generate_extract_heavy("extract_heavy.in", ops=999)
    generate_balanced("balanced.in", ops=999)
